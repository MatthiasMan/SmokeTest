...
class cgolpitch extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (inputRows == null || inputCols == null || setSButton == null || rButton == null || pButton == null || cButton == null || raButton == null || slider == null)
            return;
...
class cgolpitchGridEditor {
    ...
    onLevelInput() {
        ...
        while (counter < text.length) {
            ...
            counter = counter + 1;
            for (let index = 0; index < howManyColumns - tempColumns; index++)
                newText = newText + "0";
        }
        var cGrid = document.getElementById("containerGrid");
        //var cGrid = that.shadowRoot.getElementById('cgollGrid');
        ...
        while (cGrid.childNodes.item(0))
            cGrid.removeChild(cGrid.childNodes.item(0));
        cgolpitchGridEditor.createGameField(function(butto, y, x) {
            if (newText[y * howManyColumns + x] == "1")
                grid[y][x].setAttribute("class", "isAlive");
            else
                grid[y][x].setAttribute("class", "notAliveYet");
        });
    }
    onRandomField() {
...
setTimeout(function() {
    ...
    static createGameField(functionn) {
        var cGrid = document.getElementById("containerGrid");
        ...
        for (let y = 0; y < howManyRows; y++) {
            ...
            for (let x = 0; x < howManyColumns; x++) {
                ...
                grid[y][x].onclick = function() {
                    if (this.getAttribute("class") == "isAlive")
                        this.setAttribute("class", "notAliveYet");
                    else if (this.getAttribute("class") == "notAliveYet")
...
class cgolpitchRunEditor {
    static run() {
        if (running)
            cgolpitchRunEditor.oneEvolution();
        var timer;
        if (running)
            timer = setTimeout(cgolpitchRunEditor.run, reproductionTime);
    }
    static oneEvolution() {
        ...
        for (let index = 0; index < howManyRows; index++) {
            ...
            for (let index2 = 0; index2 < howManyColumns; index2++) {
                if (grid[index][index2].getAttribute("class") != undefined || grid[index][index2].getAttribute("class") != null) {
                    if (grid[index][index2].getAttribute("class") == "isAlive")
                        tempGrid[index][index2] = 1;
                    else if (grid[index][index2].getAttribute("class") == "wasAlive")
                        tempGrid[index][index2] = 0;
                    else
                        tempGrid[index][index2] = -1;
                }
            }
        ...
        for (let y = 0; y < howManyRows; y++) {
            for (let x = 0; x < howManyColumns; x++) {
                if (cgolpitchRunEditor.useRules(x, y))
                    stillEvolutiong = true;
            }
        }
        for (let index = 0; index < howManyRows; index++) {
            for (let index2 = 0; index2 < howManyColumns; index2++) {
                if (tempGrid[index][index2] == 1)
                    grid[index][index2].setAttribute("class", "isAlive");
                else if (tempGrid[index][index2] == 0)
                    grid[index][index2].setAttribute("class", "wasAlive");
                else
                    grid[index][index2].setAttribute("class", "notAliveYet");
            }
        }
    ...
    static useRules(x, y) {
        ...
        for (let index = -1; index < 2; index++) { //counting neighbours
            for (let index2 = -1; index2 < 2; index2++) {
                if (index != 0 || index2 != 0) {
                    if (grid[(y + index + howManyRows) % howManyRows][(x + index2 + howManyColumns) % howManyColumns].getAttribute("class") == "isAlive")
                        count++; //neighbour found
                }
            }
        }
...
class cgolpitchSupportEditor {
    ...
    onRowsChanged() {
        ...
        if (rowInput == null)
            return;
        if (Number(rowInput.value) < 101 && Number(rowInput.value) > 9)
            howManyRows = Number(rowInput.value);
    }
    onColsChanged() {
        var inputs = document.getElementsByTagName("input");
        ...
        if (colInput == null)
            return;
        if (Number(colInput.value) < 201 && Number(colInput.value) > 9)
            howManyColumns = Number(colInput.value);
    }
}
